...
test("six-panel roof appears with its cuts and can be hidden", async ({ page }) => {
  ...
  expect(model.roof_layout.clamps).toHaveLength(84);
  expect(model.roof_layout.clamps.filter((c: { kind: string }) => c.kind === "shared")).toHaveLength(0);
  expect(model.roof_layout.clamps.every((c: { panel_ids: string[] }) => c.panel_ids.length === 1)).toBe(true);
  expect(model.roof_layout.internal_support_ids).toHaveLength(5);
  ...
  await expect(note).toContainText("no washer clamps two panels");
